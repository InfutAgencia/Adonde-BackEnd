...
import { notFoundHandler, logError } from "./utils/middlewares/errorHandler";
...
app.use(
  express.urlencoded({ extended: true, limit: "50 mb", parameterLimit: 50000 })
);
...
app.get("/", (req, res) => res.status(200).send("Welcome to Adonde API"));
...
const startServer = async (port) => {
  try {
    ...
    if (!normalizedPort)
      throw new Error("No valid PORT configuration. Please check");
